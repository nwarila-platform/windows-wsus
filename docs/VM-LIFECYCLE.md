...
and run the smoke proof (S3 in `_handoff/RESTART.md` §6 / first composed run) before
any cycle uses it.
...
- If the LAN itself ever changes (subnet/gateway), that is a baseline-level change:
  ...
  `ansible/inventory/vmware.yml`, `scripts/revert-vm.sh`, and `_handoff/RESTART.md`
