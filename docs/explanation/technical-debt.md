...
- **What:** Renovate maintains the GitHub Actions, exact Python requirements, and Ansible Galaxy
  ...
  automation updates the coupled Actionlint, ShellCheck package, and Terraform CLI
  version/checksum values in `quality-tools.env`.
...
- **Progress:** 2026-08-10 — pin discovery now carries a tuple phase: actionlint from its GitHub
  ...
  from `quality-tools.env` instead of a duplicated literal. 2026-08-14 — the session-manager-plugin
  tuple retired with the SSM transport: reachability is now direct SSH, so the plugin is no longer
  installed, pinned, or discovered.
