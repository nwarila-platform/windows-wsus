...
| 4 | `MAIN \| Install WSUS Server Role (WID) And IIS HTTP Logging` | reproduce against SQL | Feature list becomes `UpdateServices-Services`, `UpdateServices-DB`, `Web-Http-Logging` with `include_management_tools: true`. The bare `UpdateServices` parent is NOT requested — corrected 2026-08-27, having first been written here as a name-for-name swap of the WID list: asking for the parent selects the WID backend by default, and Windows refuses `UpdateServices-WidDB` and `UpdateServices-DB` together, so the swapped list fails on first install. The parent still arrives as a dependency of `UpdateServices-Services`, which is the point — installed rather than requested, it brings no default backend. `UpdateServices-WidDB` inverts (see the WID table) by assertion rather than by removal: nothing in this deployment installs it, the image carries no WSUS, the host is rebuilt from that image every run, and validation refuses both names that would select the backend. A removal task would defend a state the deployment cannot reach. The `include_sub_features` prohibition survives with its reason inverted: it previously had to be omitted because `-IncludeAllSubFeature` pulls `UpdateServices-DB`; it must now be omitted because that flag pulls `UpdateServices-WidDB`. |
