...
2. **WSUS install spine — C03/C04/C05 ✅ merged (WSUS IS ALIVE: Get-WsusServer OK :8530), C06 🔄
   in-flight** (SUSDB relocation C:→E:, decomposed C06a–i; C06a-c+U1+C05r+C06d-f+**C06g** merged; **SUSDB
   NOW LIVE ON E:** (attached ONLINE + read-write, C: retained as fallback); **C06h (start services) NEXT**) → C07 END.
...
**STATE: C06g merged — SUSDB NOW LIVE ON E: (attached ONLINE + read-write; C: retained as fallback).**
`main` @ the C06g codification commit (merge `f4b39ae [audited 036d635]`). Gate reads 'relocated' (all
files on E:). Services are STILL STOPPED (C06d stopped them; nothing has restarted them yet). **NEXT:
C06h** — `win_service` state: started, loop [W3SVC, WsusService], ENSURE-STARTED (NOT relocation-gated:
the role's end state is services-running regardless of relocation; win_service state:started is
idempotent). Start order IIS→WSUS (reverse of C06d's stop). **Then C06i** — delete the C: SUSDB
originals, INDEPENDENTLY health-gated (only when SUSDB is ONLINE+read_write on E: — the C06g forward
commitment). **Then C07** — END verify (WsusService running + Get-WsusServer + :8530 + DB on E: /
content on F:). `pre-C06h` rolling snapshot from the converged post-C06g state (services stopped, SUSDB
relocated/ONLINE on E:, C: intact). Older handoff detail below is historical. `present_windows.yml`: read-only
